...
class Scene7CheckerLogic: ObservableObject {
    ...
    func checkImageForProposedName(recordID: UUID, newProposedName: String) {
        ...
        record.md5Hash = nil
        // Do NOT clear slot info here! Keep usedDetailSlots, availableDetailSlots, detailSlotChecked.
        ...
        Task {
            let checkedRecord = await self.checkedImageRecord(for: record)
            // After checking, revalidate for duplicates in case others changed meanwhile
            let otherNames2 = records
                .filter { $0.id != checkedRecord.id }
                .map { $0.proposedName.lowercased() }
            var checkedAndValidated = checkedRecord
            checkedAndValidated = validateRename(checkedAndValidated, existingNames: otherNames2)
            if let idx2 = self.records.firstIndex(where: { $0.id == checkedAndValidated.id }) {
                self.records[idx2] = checkedAndValidated
            }
        }
    }
    ...
    // Can Export/Rename?
    var canExport: Bool {
        // Only allow if all records have no renameError and at least one proposedName is different
